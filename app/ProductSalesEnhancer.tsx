...
export default function ProductSalesEnhancer(){
 ...
 function syncBranch(){const sel=document.querySelector<HTMLSelectElement>('.orderDetailsPanel select[required]');if(sel?.value&&sel.value!==branchId){setBranchId(sel.value);setCart(current=>current.filter(x=>products.some(p=>p.id===x.product_id&&p.branch_id===sel.value&&p.quantity>=x.quantity)))}}
 function productTotal(){return cart.reduce((n,x)=>n+(products.find(p=>p.id===x.product_id)?.selling_price??0)*x.quantity,0)}
 function syncSummary(){
  const summary=document.querySelector<HTMLElement>(".desktopOrderSummary .orderSummary");if(!summary)return;
  let row=summary.querySelector<HTMLElement>(".productSummaryRow");
  if(!row){row=document.createElement("div");row.className="productSummaryRow";const totalCandidates=Array.from(summary.querySelectorAll<HTMLElement>("div,p"));const totalRow=totalCandidates.find(x=>/total/i.test(x.textContent??"")&&!/subtotal/i.test(x.textContent??""));if(totalRow?.parentElement)totalRow.parentElement.insertBefore(row,totalRow);else summary.appendChild(row)}
  row.innerHTML=`<span>Products Subtotal</span><strong>${peso.format(productTotal())}</strong>`;
 }
 async function finalizePending(){const raw=sessionStorage.getItem(PENDING_KEY);if(!raw)return;let picks:Pick[]=[];try{picks=JSON.parse(raw)}catch{sessionStorage.removeItem(PENDING_KEY);return}if(!picks.length){sessionStorage.removeItem(PENDING_KEY);return}const orderId=new URLSearchParams(location.search).get("id");if(!orderId)return;const doneKey=`labaflow.products-added.${orderId}`;if(sessionStorage.getItem(doneKey)==="1"){sessionStorage.removeItem(PENDING_KEY);return}setFinalizing(true);const {error}=await supabase.rpc("add_products_to_order",{p_order_id:orderId,p_products:picks});if(error){sessionStorage.setItem("labaflow.product-sale-error",error.message);setFinalizing(false);return}sessionStorage.setItem(doneKey,"1");sessionStorage.removeItem(PENDING_KEY);sessionStorage.removeItem("labaflow.product-sale-error");location.reload()}
