...
type Org={id:string;name:string;slug:string;active:boolean;created_at:string;inventory_granted:boolean};
...
export default function PlatformAdminPage(){
  ...
  async function toggleInventoryGrant(org:Org){
    const {error}=await supabase.rpc("set_platform_organization_feature_grant",{p_organization_id:org.id,p_feature_key:"inventory",p_granted:!org.inventory_granted});
    if(error)setMessage(error.message);else{setMessage(`Product & Inventory ${org.inventory_granted?"revoked from":"granted to"} ${org.name}.`);await loadContext()}
  }
