...
export default function SharedSidebar({active}:{active?:string}){
 ...
 const section=(title:string,children:React.ReactNode)=><div className="navSection"><div className="navSectionLabel navText">{title}</div>{children}</div>;
 const hasAdmin=Boolean(p.manage_organization||access.is_platform_admin);
 return <aside data-shared-sidebar className={`sidebar sharedSidebar ${collapsed?"collapsed":""}`}><div className="brand"><div className="logoMark small">LF</div><div className="navText"><strong>LabaFlow</strong><small>{context.organization?.name??"Workspace"}</small></div><button type="button" className="sidebarToggle" onClick={toggle} title={collapsed?"Show navigation":"Hide navigation"}>{collapsed?"›":"‹"}</button></div><div className="roleBadge navText"><strong>{context.profile?.full_name||context.profile?.email||"LabaFlow User"}</strong><span>{label(access.role??"member")}{branch?` · ${branch.name}`:""}</span></div><nav>{section("Operations",<>{item("dashboard","Dashboard","/")}{item("new-order","New Order","/new-order",Boolean(p.create_orders))}{item("orders","Orders","/orders",Boolean(p.create_orders||p.process_orders||p.record_payments||p.view_reports||p.delivery_access))}</>)}{section("Customers",<>{item("customers","Customers","/customers",Boolean(p.manage_customers||p.create_orders||p.adjust_loyalty))}{item("loyalty","Loyalty","/?view=loyalty",Boolean(p.adjust_loyalty||p.view_reports))}</>)}{section("Management",<>{item("services","Services & Pricing","/?view=services",Boolean(p.manage_services))}{item("dispatch","Pickup & Delivery","/pickup-delivery",pickup&&Boolean(p.delivery_access||p.create_orders||p.view_reports))}</>)}{hasAdmin&&<div className={`navSection adminGroup ${adminOpen?"open":""}`}><button type="button" className="navGroupToggle" onClick={toggleAdmin} title="Administration"><span className="navIcon">A</span><span className="navText">Administration</span><span className="navChevron navText">{adminOpen?"⌄":"›"}</span></button>{adminOpen&&<div className="navGroupItems">{item("organization","Organization Admin","/organization",Boolean(p.manage_organization))}{item("payments","Payment Settings","/organization/payments",Boolean(p.manage_organization))}{item("loyalty-levels","Loyalty Levels","/organization/loyalty-levels",Boolean(p.manage_organization))}{item("promos","Promo Codes","/organization/promos",Boolean(p.manage_organization))}{item("delivery-pricing","Delivery Pricing","/organization/delivery-pricing",Boolean(p.manage_organization))}{item("platform","Platform Admin","/admin",Boolean(access.is_platform_admin))}</div>}</div>}</nav><div className="sidebarFooter"><button title="Sign out" onClick={()=>supabase.auth.signOut().then(()=>location.href="/")}><span className="navText">Sign out</span><span className="navIcon">↪</span></button></div></aside>
}
