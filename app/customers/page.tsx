...
export default function CustomersPage(){
 ...
 function startNewAddress(){setAddressForm(blankAddress);setDistances(x=>Object.fromEntries(Object.entries(x).filter(([k])=>!k.startsWith("new:"))))}
 async function saveAddress(e:FormEvent){e.preventDefault();if(!editing)return;let addressId=addressForm.id;const distanceKeyPrefix=addressForm.id||"new";
  ...
  for(const b of branches){const raw=distances[`${distanceKeyPrefix}:${b.id}`];if(raw!==undefined&&raw!==""){const {error}=await supabase.rpc("set_customer_address_distance",{p_address_id:addressId,p_branch_id:b.id,p_distance_km:Number(raw)});if(error){setMessage(error.message);return}}}
