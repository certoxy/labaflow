...
export default function AdminSubpagesEnhancer(){
 useEffect(()=>{
  ...
  const enhance=()=>{
   ...
   if(path==="/admin"){const actions=main.querySelector<HTMLElement>(":scope > .panelHead .headerActions");if(actions&&!actions.querySelector('[data-system-alerts-link="1"]')){const btn=document.createElement("button");btn.className="secondary";btn.type="button";btn.dataset.systemAlertsLink="1";btn.textContent="System Alerts";btn.onclick=()=>{location.href="/admin/system-alerts"};actions.prepend(btn)}if(actions&&!actions.querySelector('[data-system-restore-link="1"]')){const btn=document.createElement("button");btn.className="secondary";btn.type="button";btn.dataset.systemRestoreLink="1";btn.textContent="System Restore";btn.onclick=()=>{location.href="/admin/system-restore"};actions.prepend(btn)}}
   if(path==="/admin/subscriptions"){const actions=main.querySelector<HTMLElement>(":scope > .panelHead .headerActions");if(actions&&!actions.querySelector('[data-upgrade-requests-link="1"]')){const btn=document.createElement("button");btn.className="primary";btn.type="button";btn.dataset.upgradeRequestsLink="1";btn.textContent="Upgrade Requests";btn.onclick=()=>{location.href="/admin/upgrade-requests"};actions.prepend(btn)}if(actions&&!actions.querySelector('[data-system-restore-link="1"]')){const btn=document.createElement("button");btn.className="secondary";btn.type="button";btn.dataset.systemRestoreLink="1";btn.textContent="System Restore";btn.onclick=()=>{location.href="/admin/system-restore"};actions.prepend(btn)}}
  };
