...
import {isAppleDevice,loadPrinterSettings,printBluetoothReceipt,type ReceiptPrintFormat} from "../../lib/printerSettings";

type CustomerRef={full_name:string;customer_code:string;loyalty_points?:number};
type BranchRef={id:string;name:string;code?:string|null;address?:string|null;phone?:string|null};
type Order={id:string;order_code:string;branch_id:string;customer_id:string|null;status:string;payment_status:string;subtotal:number;discount:number;total:number;amount_paid:number;loyalty_points_redeemed?:number;notes:string|null;created_at:string;customers:CustomerRef|null;branch?:BranchRef|null};
type OrderItem={id:string;service_id:string;service_name:string;pricing_unit:string;quantity:number;unit_price:number;line_total:number;notes:string|null;loyalty_points_per_unit:number;loyalty_points_earned:number};
...
export default function OrderDetailsPage(){
 ...
 async function load(){
  ...
  if(local){
   ...
   setOrder(normalized);
   const localLoyalty=detail.data.loyalty??{};
   setLoyalty({balance:Number(localLoyalty.balance??raw.customers?.loyalty_points??0),earned:Number(localLoyalty.earned??0),used:Number(localLoyalty.used??raw.loyalty_points_redeemed??0)});
   setItems((detail.data.items??[]).map((x:any)=>({...x,quantity:Number(x.quantity),unit_price:Number(x.unit_price),line_total:Number(x.line_total),loyalty_points_per_unit:Number(x.loyalty_points_per_unit??0),loyalty_points_earned:Number(x.loyalty_points_earned??0)})));
  ...
  const [a,o,i,p,g,productResult,brand,loyaltyResult]=await Promise.all([
   supabase.rpc("get_current_access_context"),
   supabase.from("laundry_orders").select("id,order_code,branch_id,customer_id,status,payment_status,subtotal,discount,total,amount_paid,loyalty_points_redeemed,notes,created_at,customers(full_name,customer_code,loyalty_points),branches(id,name,code,address,phone)").eq("id",orderId).maybeSingle(),
   supabase.from("laundry_order_items").select("id,service_id,quantity,unit_price,line_total,notes,loyalty_points_per_unit,loyalty_points_earned,services(name,pricing_unit)").eq("order_id",orderId).order("created_at"),
   ...
   supabase.from("loyalty_transactions").select("transaction_type,points").eq("reference_type","laundry_order").eq("reference_id",orderId)
  ]);
  ...
  const loyaltyRows=loyaltyResult.error?[]:loyaltyResult.data??[];
  setLoyalty({balance:Number(normalized.customers?.loyalty_points??0),earned:loyaltyRows.filter((x:any)=>x.transaction_type==="earn").reduce((n:number,x:any)=>n+Math.max(Number(x.points??0),0),0),used:loyaltyRows.filter((x:any)=>x.transaction_type==="redeem").reduce((n:number,x:any)=>n+Math.abs(Math.min(Number(x.points??0),0)),0)||Number(normalized.loyalty_points_redeemed??0)});
  if(!brand.error)setBranding(brand.data);
  setItems((i.data??[]).map((x:any)=>({...x,service_name:Array.isArray(x.services)?x.services[0]?.name??"Service":x.services?.name??"Service",pricing_unit:Array.isArray(x.services)?x.services[0]?.pricing_unit??"":x.services?.pricing_unit??"",quantity:Number(x.quantity),unit_price:Number(x.unit_price),line_total:Number(x.line_total),loyalty_points_per_unit:Number(x.loyalty_points_per_unit??0),loyalty_points_earned:Number(x.loyalty_points_earned??0)})));
 ...
 return <main className="workspace ordersExperience"><header><div><p className="eyebrow">ORDER CHECKOUT</p><h1>{order.order_code}</h1><p className="muted">Review services, collect payment, and print the customer receipt.</p></div><div className="headerActions"><button className={defaultPrint==="standard"?"primary":"secondary"} onClick={()=>printReceipt("standard")}>Print / PDF</button><button className={defaultPrint==="58mm"?"primary":"secondary"} onClick={()=>printReceipt("58mm")}>{isAppleDevice()?"AirPrint 58mm":"Print 58mm"}</button><button className="secondary" onClick={()=>location.href="/new-order"}>+ New Order</button><button className="secondary" onClick={()=>location.href="/orders"}>All Orders</button></div></header>{message&&<p className="notice">{message}</p>}
 ...
 {paid&&<section className="panel"><p className="notice"><strong>✓ Payment complete.</strong> This order has been fully paid.</p><div className="actions"><button className={defaultPrint==="standard"?"primary":"secondary"} onClick={()=>printReceipt("standard")}>Print / PDF</button><button className={defaultPrint==="58mm"?"primary":"secondary"} onClick={()=>printReceipt("58mm")}>{isAppleDevice()?"AirPrint 58mm":"Print 58mm"}</button><button className="secondary" onClick={()=>location.href="/orders"}>Continue to Order Management</button><button className="secondary" onClick={()=>location.href="/new-order"}>Create Another Order</button></div></section>}
 </main>
}
