  ...
  const patchedOpen:typeof window.open=((...args:any[])=>{
   ...
   try{
    ...
    receiptWin.document.write=((...chunks:string[])=>{
     ...
     html=html.replace("<header><h1>LabaFlow</h1>",`<div class="receiptActions noPrint"><button onclick="location.assign('/order-details?id=${encodeURIComponent(orderId)}')">← Back to Order</button><button onclick="downloadPdf()">↓ Download PDF</button><button onclick="shareReceipt()">↗ Share Image</button><button onclick="window.print()">🖨 Print</button></div><header><img class="receiptLogo" src="/labaflow-icon.svg" alt="LabaFlow"><h1>LabaFlow</h1>`);
