...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const [{data:event},{data:admins}]=await Promise.all([admin.from("system_error_events").select("id,severity,message,route,error_code,occurrence_count,last_occurred_at,organizations(name),profiles(email,full_name)").eq("id",reported.event_id).single(),admin.from("platform_admins").select("profiles(email)").eq("active",true)]);
  const recipients=(admins??[]).map((a:any)=>a.profiles?.email).filter(Boolean);if(!event||!recipients.length)return json({reported:true,event_id:reported.event_id,emailed:false,email_warning:"No Platform Admin email recipient"});
  const org=(event as any).organizations?.name??"Unknown organization",affected=(event as any).profiles?.email??"Unknown user";
