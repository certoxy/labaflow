-- Let organization administrators choose a plan during the free trial without
-- creating a PayMongo charge. Hosted checkout becomes available when the trial
-- has ended, and reuses the saved plan selection.
begin;

create or replace function public.schedule_subscription_after_trial(
  p_plan_key text,
...
begin
  ...
  if p_billing_cycle not in ('monthly','yearly') then
    raise exception 'Select monthly or yearly billing';
  end if;
  if not exists(
    select 1 from public.subscription_plans
    where plan_key=lower(trim(p_plan_key)) and active and monthly_price is not null
  ) then
    raise exception 'Select a paid plan';
  end if;
  ...
  if v_status is distinct from 'trialing' or v_trial_ends_at is null or now()>=v_trial_ends_at then
    raise exception 'The free trial has ended. Continue to PayMongo to activate this plan';
  end if;

  select id into v_request_id
  from public.subscription_upgrade_requests
  where organization_id=v_org and request_type='plan_upgrade' and status='pending'
  for update;
  ...
  if v_request_id is null then
    insert into public.subscription_upgrade_requests(
      ...
      v_org,'plan_upgrade',lower(trim(p_plan_key)),
      nullif(trim(coalesce(p_billing_contact,'')),''),
      nullif(lower(trim(coalesce(p_billing_email,''))),''),'paymongo_after_trial',
    ...
    set requested_plan_key=lower(trim(p_plan_key)),
        billing_contact=nullif(trim(coalesce(p_billing_contact,'')),''),
        billing_email=nullif(lower(trim(coalesce(p_billing_email,''))),''),
        payment_method='paymongo_after_trial',
        notes=nullif(trim(coalesce(p_notes,'')),''),
        requested_by=auth.uid(),
        billing_cycle=p_billing_cycle,
        product_inventory_addon=p_product_inventory_addon,
        created_at=now()
...
grant execute on function public.schedule_subscription_after_trial(text,text,boolean,text,text,text) to authenticated;
...
begin
  ...
  if v_status='trialing' and now()<v_trial_ends_at then
    raise exception 'Payment becomes available after your free trial ends on %',
      to_char(v_trial_ends_at at time zone 'UTC','Mon DD, YYYY');
  end if;
  ...
  if exists(
    ...
    where organization_id=v_org and status in ('creating','awaiting_payment')
  ) then
    raise exception 'A PayMongo checkout is already awaiting payment';
  end if;
  ...
  if v_request_id is null then
    ...
    update public.subscription_upgrade_requests
    set requested_plan_key=v_plan.plan_key,
        billing_contact=nullif(trim(coalesce(p_billing_contact,'')),''),
        billing_email=nullif(lower(trim(coalesce(p_billing_email,''))),''),
