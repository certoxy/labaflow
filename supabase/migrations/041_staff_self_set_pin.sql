...
begin
  ...
  if p_branch_id is not null and not exists(select 1 from public.branches b where b.id=p_branch_id and b.organization_id=v_org and b.active=true) then raise exception 'Invalid branch'; end if;
...
begin
  ...
  update public.local_staff_sessions ls
  set revoked_at=now()
  where ls.staff_id=p_staff_id and ls.revoked_at is null;
...
begin
  ...
  where o.slug=lower(trim(p_organization_slug)) and o.active=true
  limit 1;
...
begin
  select o.* into v_org from public.organizations o where o.slug=lower(trim(p_organization_slug)) and o.active=true limit 1;
  if v_org.id is null then raise exception 'Invalid organization, username, or PIN'; end if;
  select s.* into v_staff from public.local_staff_accounts s
  where s.organization_id=v_org.id and s.username=lower(trim(p_username)) and s.active=true limit 1;
  ...
  if v_staff.branch_id is not null then select b.* into v_branch from public.branches b where b.id=v_staff.branch_id and b.active=true; end if;
...
begin
  ...
  select
    ...
    s.last_login_at,
    s.created_at,
    (s.pin_hash is not null) as pin_configured
  from public.local_staff_accounts s
